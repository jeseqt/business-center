-- Fix process_wallet_transaction RPC: handle external_user_id type mismatch (uuid vs text)
...
BEGIN
    ...
    -- Fix: cast both to text to avoid "operator does not exist: uuid = text" error
    SELECT id INTO v_platform_user_id
    FROM public.platform_users
    WHERE external_user_id::text = _user_id::text AND app_id = _app_id
    LIMIT 1;
