...
BEGIN
    ...
    -- 预计算密码哈希 (避免循环中重复计算)
    -- 尝试使用 extensions schema，如果失败则回退到默认路径（通过 search_path）
    ...
    LOOP
        ...
        IF v_user_id IS NULL THEN
            ...
            INSERT INTO auth.identities (
                ...
                gen_random_uuid(), -- 生成新的 identity ID
                v_user_id, -- user_id
                jsonb_build_object('sub', v_user_id, 'email', r.email, 'email_verified', true, 'phone_verified', false),
                'email',
                r.email, -- provider_id for email provider is the email itself
                NULL,
                now(),
                now()
            );
