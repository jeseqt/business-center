...
-- Fix process_wallet_transaction to cast text input to uuid for comparison with platform_users.external_user_id
...
BEGIN
    ...
    -- Cast _user_id to uuid to match external_user_id column type
    BEGIN
        SELECT id INTO v_platform_user_id
        FROM public.platform_users
        WHERE external_user_id = _user_id::uuid AND app_id = _app_id
        LIMIT 1;
    EXCEPTION WHEN invalid_text_representation THEN
        -- If _user_id is not a valid uuid, return error immediately
