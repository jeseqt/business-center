...
-- Fix process_wallet_transaction to accept text user_id (matching platform_users.external_user_id)
CREATE OR REPLACE FUNCTION public.process_wallet_transaction(
    _user_id text,          -- Changed from uuid to text
...
BEGIN
    ...
    -- No need to cast _user_id to text anymore as it is already text
    SELECT id INTO v_platform_user_id
    FROM public.platform_users
    WHERE external_user_id = _user_id AND app_id = _app_id
    LIMIT 1;
